...
// `emp` son los datos de la empresa guardados en Configuración (/api/empresa).
// Antes estaban escritos a mano acá adentro: por eso cambiarlos en Configuración
// no se reflejaba nunca en el PDF impreso.
function imprimirCotizacion(c, emp = {}) {
  const e = {
    nombre: 'RMG Parts', razon_social: 'RMG Parts SpA',
    email: 'ventas@rmgautoparts.cl', telefono: '+56 9 7448 8647',
    ciudad: 'Santiago, Región Metropolitana',
    pie_pagina: 'Distribución mayorista B2B · Santiago RM',
    ...Object.fromEntries(Object.entries(emp || {}).filter(([, v]) => v !== null && v !== '')),
  }
  const direccionLinea = [e.direccion, e.comuna].filter(Boolean).join(', ') || e.ciudad
...
<html lang="es"><head>
<meta charset="UTF-8">
<title>Cotización ${c.numero} — ${e.nombre}</title>
<style>
...
<div class="page">
  ...
  <div class="cond-grid">
    ...
    <div class="cond-box">
      ...
      <div style="margin-top:10px">
        ...
        ${e.banco_nombre ? `<div class="bank-item"><strong>${e.banco_nombre}</strong></div>` : ''}
        ${e.banco_numero ? `<div class="bank-item">${e.banco_tipo_cuenta || 'Cuenta'} N° ${e.banco_numero}</div>` : ''}
        ${(e.banco_rut || e.banco_titular) ? `<div class="bank-item">${e.banco_rut ? `RUT: ${e.banco_rut} · ` : ''}${e.banco_titular || e.razon_social}</div>` : ''}
        ${(e.banco_email || e.email) ? `<div class="bank-item" style="color:#718096;font-size:10.5px">Enviar comprobante a ${e.banco_email || e.email}</div>` : ''}
      </div>
    </div>
  </div>
  ...
  <div class="footer">
    <div class="left">${e.nombre}${e.pie_pagina ? ` · ${e.pie_pagina}` : ''}</div>
...
export default function CotizacionesPage() {
  ...
  // Membrete de los documentos — se edita en Configuración → Empresa.
                      ...
                      <td className="px-4 py-3">
                        <div className="flex gap-1.5 items-center" onClick={e => e.stopPropagation()}>
                          <button className="btn-secondary text-xs px-2 py-1 flex items-center gap-1" onClick={async e => {
                            ...
                            imprimirCotizacion(full, empresa)
                          }}>
                            <Printer size={11}/> PDF
                          </button>
