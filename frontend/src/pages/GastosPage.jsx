...
import { Plus, X, TrendingDown, ChevronDown, ChevronUp } from 'lucide-react'
...
const GASTOS_INIT = [
  { id: 1, fecha: '2026-06-02', categoria: 'combustible',    monto: 45_000,    descripcion: 'Gasolina furgón despacho',          comprobante: 'BOL-001'  },
  { id: 2, fecha: '2026-06-05', categoria: 'bodega',          monto: 350_000,   descripcion: 'Arriendo bodega junio',              comprobante: 'FAC-A-123'},
  { id: 3, fecha: '2026-06-08', categoria: 'logistica',       monto: 85_000,    descripcion: 'Flete despacho Quilicura',           comprobante: 'BOL-002'  },
  { id: 4, fecha: '2026-06-10', categoria: 'administrativo',  monto: 1_200_000, descripcion: 'Sueldos y comisiones vendedores',    comprobante: 'COMP-001' },
  { id: 5, fecha: '2026-06-12', categoria: 'marketing',       monto: 120_000,   descripcion: 'Meta Ads — semana 2 junio',          comprobante: 'FAC-B-456'},
  { id: 6, fecha: '2026-06-15', categoria: 'combustible',     monto: 52_000,    descripcion: 'Gasolina furgón + camioneta',        comprobante: 'BOL-003'  },
  { id: 7, fecha: '2026-06-18', categoria: 'logistica',       monto: 95_000,    descripcion: 'Flete despacho MetroMov Buses',      comprobante: 'BOL-004'  },
  { id: 8, fecha: '2026-06-20', categoria: 'otros',            monto: 35_000,    descripcion: 'Materiales de embalaje',             comprobante: 'BOL-005'  },
  { id: 9, fecha: '2026-06-22', categoria: 'marketing',       monto: 120_000,   descripcion: 'Meta Ads — semana 3-4 junio',        comprobante: 'FAC-B-457'},
]
...
export default function GastosPage() {
  ...
  const handleSubmit = (e) => {
    ...
    const nuevo = { ...form, id: Date.now(), monto: Number(form.monto) }
    ...
    <div className="space-y-5 animate-fade-in">
      ...
      <div className="flex justify-between items-start">
        <div>
          ...
          <p className="text-sm mt-0.5" style={{ color: 'var(--rmg-muted)' }}>Control de egresos operacionales — Junio 2026</p>
        </div>
        <button
        ...
        <div className="rmg-card p-5 animate-fade-in">
          ...
          <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div className="md:col-span-3 flex gap-3 justify-end pt-1">
              <button type="button" onClick={() => { setShowForm(false); setForm(FORM_INIT) }}
        ...
        <div className="h-3 rounded-full" style={{ background: 'rgba(255,255,255,0.06)' }}>
          <div className="h-3 rounded-full transition-all duration-700"
            style={{
              ...
              background: pctPresupuesto > 90
                ? 'var(--rmg-red)'
                : pctPresupuesto > 75
                  ? 'var(--rmg-gold)'
                  : 'linear-gradient(90deg, var(--rmg-blue), var(--rmg-teal))',
            }} />
        </div>
      </div>
      ...
      <div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
          {resumen.map(cat => (
            <button key={cat.key}
              onClick={() => setFiltro(filtro === cat.key ? '' : cat.key)}
              ...
              <div className="font-black precio-clp" style={{ color: cat.color, fontFamily: 'Inter Tight, sans-serif' }}>
                {formatCLP(cat.total)}
              </div>
      ...
      <div className="rmg-card overflow-hidden">
        <div className="px-5 py-3 border-b flex justify-between items-center"
          ...
          <span className="font-bold text-sm">
            {filtro
              ? `${getCat(filtro).icon} ${getCat(filtro).label} — ${gastosFiltrados.length} registros`
              : `Todos los gastos — ${gastos.length} registros`
            }
          </span>
          {filtro && (
              ...
              .map((g, i) => {
                ...
                return (
                  <tr key={g.id}
                    ...
                    <td className="px-4 py-3 font-bold precio-clp text-right" style={{ color: 'var(--rmg-off)' }}>
                      {formatCLP(g.monto)}
                    </td>
                  </tr>
                )
          ...
          {gastosFiltrados.length > 0 && (
            <tfoot>
              <tr style={{ background: 'rgba(255,255,255,0.03)', borderTop: '1px solid rgba(56,182,255,0.1)' }}>
                <td colSpan={4} className="px-4 py-3 text-xs font-semibold uppercase tracking-wider"
                  style={{ color: 'var(--rmg-muted)' }}>Subtotal {filtro ? getCat(filtro).label : 'mes'}</td>
                <td className="px-4 py-3 font-black precio-clp text-right text-base"
                  style={{ color: 'var(--rmg-blt)', fontFamily: 'Inter Tight, sans-serif' }}>
        ...
        {gastosFiltrados.length === 0 && (
          <div className="py-12 text-center" style={{ color: 'var(--rmg-muted)' }}>
            ...
            <p className="text-sm">Sin gastos registrados en esta categoría</p>
          </div>
        )}
      </div>
    </div>
