...
export default function ComprasPage() {
  ...
  const renderAcciones = (oc) => {
    ...
    if (oc.estado === 'borrador' && !oc.pagada) {
      if (esAdmin) {
        botones.push(
          <button key="edit" onClick={() => setEditando({ ...oc })} className="p-1.5 rounded hover:bg-black/5" style={{ color: 'var(--rmg-muted)' }} title="Editar"><Pencil size={13}/></button>,
  ...
  return (
    <div className="space-y-5 animate-fade-in">
      ...
      <div className="rmg-card overflow-hidden">
        <div className="px-5 py-3 border-b" style={{ borderColor: 'rgba(56,182,255,0.1)', background: 'rgba(15, 35, 60,0.02)' }}>
              ...
              {isLoading
                ...
                : ordenes.map((oc, idx) => {
                    ...
                    return (
                      <>
                        <tr key={oc.id}
                          style={{ borderBottom: isExp ? 'none' : '1px solid rgba(15, 35, 60,0.04)', background: idx % 2 ? 'transparent' : 'rgba(15, 35, 60,0.01)' }}
                          ...
                          <td className="px-4 py-3">
                            <button onClick={() => setExpandida(isExp ? null : oc.id)} className="p-1 rounded hover:bg-black/5" style={{ color: 'var(--rmg-muted)' }}>
                              {isExp ? <ChevronUp size={14}/> : <ChevronDown size={14}/>}
                            </button>
                          </td>
                        </tr>
                        {isExp && (
                          <tr key={`${oc.id}-detail`} style={{ borderBottom: '1px solid rgba(15, 35, 60,0.04)' }}>
                              ...
                              <table className="w-full text-xs">
                                ...
                                <tbody>
                                  {(oc.items || []).map((item, ii) => {
                                    ...
                                    return (
                                      <tr key={ii} style={{ borderTop: '1px solid rgba(15, 35, 60,0.03)' }}>
